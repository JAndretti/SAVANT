...
That ranking is not size-independent. On [the CVRPLib sets](#the-cvrplib-sets-x-xl-and-xml100),
measured against published optima and best known solutions, HGS keeps its lead at
n = 100, the two are indistinguishable over n = 100–1000, and on Set XL
(n = 1047–10000, 20 CPU-seconds per instance) **SAVANT is ahead by 2.2 % and wins 94
of the 97 instances both solve** — plus the three where HGS reaches no feasible
solution at all. That last result is about HGS's behaviour at a short budget, not
about SAVANT being competitive at scale: the XL paper's reference solvers, given two
hours, all sit between +0.16 % and +1.45 % of the best known solutions where SAVANT
at 18 seconds sits at +2.07 %. The
[XL section](#what-the-xl-paper-reports) reports them side by side.
...
- [Getting started](#getting-started) · [Results](#results) · [against HGS run here](#against-hgs-run-here) · [the CVRPLib sets](#the-cvrplib-sets-x-xl-and-xml100) · [LKH-3 and AILS-II](#two-more-reference-solvers-lkh-3-and-ails-ii)
...
### From nothing to a four-way comparison

```bash
sh tools/setup.sh check          # what is installed, what is missing
sh tools/setup.sh                # fetch every dataset, build every solver
sh tools/compare_all.sh X 24 10  # SAVANT + HGS + LKH-3 + AILS-II, ~4 min
```

It also takes `data`, `solvers` and `jdk` to do one part only.

`setup.sh` is POSIX `sh`, runs on Linux and macOS alike, and is idempotent —
anything already present is left alone. It **degrades rather than failing**:
without OpenMP it builds SAVANT single-threaded, without a JDK it skips AILS-II
and says so. Every check probes for the tool rather than branching on `uname`,
and every missing tool is reported with the install line for each common
package manager.
...
|---|---|---|
| C compiler, `make` | SAVANT, LKH-3 | `apt install build-essential` · `dnf install gcc make` · `pacman -S base-devel` · `xcode-select --install` |
| `cmake` | HGS-CVRP | `apt`/`dnf`/`pacman`/`brew install cmake` — or, **without root**, `uv tool install cmake` |
| `curl`, `git` | fetching everything | in every distribution's base package set; ship with macOS |
| a `.7z` reader | the CVRPLib archives | any one of `bsdtar` (`apt install libarchive-tools` · `dnf install bsdtar` · `pacman -S libarchive` · already on macOS), the 7-Zip CLI, or **without root** the pure-Python `py7zr`, which `setup.sh` pulls in on the fly via `uv run --with` |
| `python3` + `uv` | the `tools/` scripts | plain `python3` works too; no third-party package is needed except `pypdf` for `parse_xl_bks.py` and `py7zr` if nothing else can read `.7z` |
| OpenMP | SAVANT's parallel loop **(optional)** | GCC and LLVM clang have it built in; Apple clang needs `brew install libomp`. `make auto` sorts this out by trying to compile, and falls back to `make serial` |
| a JDK | AILS-II **(optional)** | `apt install default-jdk` · `dnf install java-latest-openjdk-devel` · `pacman -S jdk-openjdk` · `brew install openjdk` — or, **without root**, `sh tools/setup.sh jdk`, which unpacks an Eclipse Temurin JDK into `external/jdk` |

A JRE is not enough: AILS-II ships as source, so the build needs `javac` and
`jar`. `setup.sh` checks for `javac` for that reason, and `run_ails.py` looks
for a launcher in `external/jdk`, then `$JAVA_HOME`, then `PATH`, then the
usual per-platform locations.

Disk: ~300 MB of instances (10 000 NeuOpt × 3 sizes, 10 000 XML100, X, XL) and
~100 MB of solver sources, plus ~330 MB if `setup.sh jdk` fetches a JDK.
Nothing under `data/`, `external/` or `results/` is tracked by git; all of it is
regenerated by `setup.sh`.

The whole of `setup.sh` — every dataset, all four solvers — plus
`compare_all.sh` on X, XML100 and XL and the `tests/` suite have been run on
Ubuntu 24.04 (GCC 13.3, OpenJDK 21) **with no root access at all**: `cmake` from
`uv tool install`, `py7zr` in place of `bsdtar`, and a Temurin JDK unpacked into
`external/jdk`. The measured results elsewhere in this README come from the
author's 12-core macOS laptop and were not re-measured here.

#### What `compare_all.sh` does

`sh tools/compare_all.sh [SET] [N_INSTANCES] [SECONDS]`, e.g.

```bash
sh tools/compare_all.sh X 24 10        # quick check, ~4 min
sh tools/compare_all.sh X 100 10       # the whole Set X, ~15 min
sh tools/compare_all.sh XML100 500 1   # optimality gaps against proven optima
sh tools/compare_all.sh XL 100 20      # large instances (4 jobs: HGS needs 2.3 GiB each)
```

It runs the four solvers **sequentially** — each saturates the machine, so
overlapping them would corrupt every timing — validates all four with
`validate.py`, and prints the gap to the set's optima or BKS. SAVANT takes a
step count rather than a time, so its budget is calibrated by a 1M-step probe
run and scaled to the target CPU seconds; the probe is kept and appears in the
table as a second, cheaper data point.

Real output, first 24 Set X instances at 10 s each:

```
### validation (recomputed from the coordinates, no code shared with any solver)
..._HGS_X_n24_t10       24 instance(s) checked, 0 error(s)
..._LKH_X_n24_t10       24 instance(s) checked, 0 error(s)
..._AILS_X_n24_t10      24 instance(s) checked, 0 error(s)
..._SAVANT_X_n24_t10    24 instance(s) checked, 0 error(s)

### gap to data/cvrplib/X_bks.csv
..._AILS_X_n24_t10        mean gap : +0.139 %
..._HGS_X_n24_t10         mean gap : +0.243 %
..._SAVANT_X_n24_t10      mean gap : +0.569 %
..._LKH_X_n24_t10         mean gap : +2.896 %
```

For the NeuOpt sets, which are continuous and therefore out of LKH-3's reach,
use `sh tools/compare_hgs.sh 1000` instead.

### The individual pieces

```bash
make auto                               # probe for OpenMP, pick the right target
...
# the CVRPLib sets, scored against their optima / BKS (these are ROUNDED)
python3 tools/fetch_cvrplib.py --verify        # X, XL, XML100
uv run --with py7zr tools/fetch_cvrplib.py --verify   # same, with no .7z tool installed
sh tools/compare_cvrplib.sh X
python3 tools/gap_to_bks.py results/<run> data/cvrplib/X_bks.csv
```

`make` has four other targets: `serial` (no OpenMP), `macos` (Apple clang plus
Homebrew's keg-only `libomp`, which needs `-Xpreprocessor` and explicit paths),
`repro` (bit-reproducible, `-ffp-contract=off`) and `debug` (ASan/UBSan).
`make auto` tries the plain `-fopenmp` build, then `macos`, then `serial`, and
prints which one it settled on — so nothing here has to know what OS it is on.
...
│   ├── fetch_cvrplib.py    download CVRPLib X / XL / XML100 + their references
│   ├── run_hgs.py          drives HGS-CVRP into the same run-directory layout
│   ├── run_lkh.py          the same for LKH-3
│   ├── run_ails.py         the same for AILS-II (needs the patch below)
│   ├── ails_solution_output.patch  makes AILS-II write its routes
...
│   ├── gap_to_bks.py       gap of a run to the CVRPLib optima / BKS
│   ├── parse_xl_bks.py     the XL paper's tables -> baseline/xl_*.csv
│   ├── setup.sh            toolchain check, then fetch datasets + build solvers
│   ├── compare_all.sh      all four solvers on one CVRPLib set, validated + scored
│   ├── compare_hgs.sh      the SAVANT-vs-HGS budget sweep on the NeuOpt sets
│   └── compare_cvrplib.sh  the same on a CVRPLib set, scored against its BKS
...
├── sweep/                the parameter study — see sweep/README.md
│   ├── run_sweep.sh        468 configurations x 5 seeds x 2 budget tiers
│   ├── analyze_sweep.py    figures + report.tex + report.pdf
│   └── report.pdf          read this one
...
### The CVRPLib sets: X, XL and XML100

The NeuOpt sets are uniform: 100 customers, coordinates drawn in [0,1]², one
distribution. Three CVRPLib sets test the opposite, and all three ship reference
solutions, so the measurement is against an optimum or a BKS rather than against
another heuristic:

| set | reference | instances | n | what it stresses |
|---|---|---:|---|---|
| **XML100** (Queiroga et al., 2022) | **proven optima** | 10000 | 100 | 378 combinations of depot placement, clustering, demand law and route length |
| **X** (Uchoa et al., 2017) | BKS | 100 | 100–1000 | the standard heterogeneous benchmark |
| **XL** (Queiroga et al., 2026) | BKS | 100 | 1047–10000 | scale |

```bash
python3 tools/fetch_cvrplib.py --verify      # all three, ~90 s, 250 MB
sh tools/compare_cvrplib.sh X                # sweep one set, both solvers
python3 tools/gap_to_bks.py results/<run> data/cvrplib/X_bks.csv
```

`fetch_cvrplib.py` writes, per set, the `.vrp`/`.sol` files as distributed, a
`.cvrpb` bundle in sorted-name order so `validate.py` needs no change, and a
`<SET>_bks.csv` holding the reference cost of every instance.

**These sets are rounded.** They follow the TSPLIB EUC_2D convention —
distances to the nearest integer — where the NeuOpt sets must not be rounded at
all. `--verify` settles it by measurement rather than by assumption: it
recomputes every shipped reference cost under both conventions and reports
which one reproduces it. Rounded matches exactly on 100/100 of Set X, 100/100
of XL and 9909/9909 of the intact XML100 references; float never matches. So
`cw` needs `--round`, and `run_hgs.py --round` defaults to `auto`, infers the
convention from the coordinates and prints its decision on every run.

**91 of the 10000 XML100 `.sol` files are damaged as distributed** — customers
unserved or listed twice; one covers 80 of its 100 customers in 3 routes. Their
`Cost` fields still look like the published optima, so `fetch_cvrplib.py` keeps
the cost and marks the route list with `ref_ok=0`. Checking feasibility *before*
the convention matters: a corrupt file otherwise reads as evidence against
rounding, which is exactly how it first appeared here.

#### Results

Matched on CPU seconds per instance, every run checked by `validate.py`:

| set | solver | budget | cpu s/inst | gap to reference |
|---|---|---|---:|---:|
| XML100 (1000 inst.) | HGS | `-t 0.25` | 0.253 | **+0.277 %** |
| XML100 (1000 inst.) | SAVANT | 1M × 10 | 0.360 | +0.500 % |
| X | SAVANT | 1M × 10 | 0.38 | +2.246 % |
| X | SAVANT | 10M × 10 | 3.66 | +1.230 % |
| X | HGS | `-t 10` | 10.01 | **+0.923 %** |
| X | SAVANT | 30M × 10 | 10.95 | +0.937 % |
| XL | HGS | `-t 20` | 20.28 | +4.365 % *(97/100)* |
| XL | SAVANT | 130M × 1 | 18.24 | **+2.073 %** *(100/100)* |

XL is scored against `baseline/xl_bks.csv`, not against the `.sol` files in the
archive — see [the XL paper's own numbers](#what-the-xl-paper-reports) for why
those two differ.

Paired per instance by `tools/paired_gap.py`, the ranking **crosses over with
instance size**:

| set | n | SAVANT vs HGS, matched CPU | SAVANT wins |
|---|---|---:|---:|
| NeuOpt CVRP-100 | 100 | +0.243 % ± 0.015 % | 27.5 % |
| XML100 | 100 | +0.222 % ± 0.013 % | 21.1 % |
| Set X | 100–1000 | +0.018 % ± 0.080 % *(tie, p = 1)* | 48 % |
| Set XL | 1047–10000 | **−2.203 % ± 0.168 %** | **96.9 %** |

On XL, SAVANT wins 94 of the 97 instances both solved, and solves the three
HGS could not.

Two qualifications, because that last line is easy to over-read.

**The comparison is at 20 CPU-seconds.** The XL best known solutions come from
120 CPU-hours per instance — 60 runs of two hours. Twenty seconds is a very
short budget at n = 10000, so this measures *anytime behaviour under a tight
budget*, not asymptotic quality. HGS spends a large part of that budget just
building its initial population; given hours it would look very different.

**HGS's failures are feasibility, not quality.** On the three XL instances it
returned nothing, and on X-n524-k153 at `-t 2`, no *feasible* solution was
reached inside the budget, so `main.cpp:30` writes no file and still exits 0.
All four are extreme-fleet instances — 155, 1012, 1932 and 2031 routes at the
reference. HGS reaches feasibility through penalty management, which needs
time; SAVANT rejects any capacity-violating move outright, so it is feasible
from the Clarke & Wright construction onwards and structurally cannot fail this
way. That is a genuine advantage of the design, and it is worth more than the
quality gap on these instances.

Also note HGS's memory: a dense n × n double matrix plus its granular
structures peaks at a measured **2.3 GiB at n = 10000**, about 3× the naive
n²·8 figure. Twelve concurrent jobs would want ~27 GiB, hence `--jobs 4` on XL.

#### What the XL paper reports

> Queiroga, Martinelli, Subramanian, Uchoa, *The XL Instances and the CVRPLib
> Best Known Solution Challenge*, arXiv:2601.11467v2

`tools/parse_xl_bks.py` reads that paper's LaTeX source and writes two files:

| file | content |
|---|---|
| `baseline/xl_bks.csv` | per instance: characteristics, **initial** BKS (pre-challenge) and **final** BKS (post-challenge), with the method and team that found each |
| `baseline/xl_solvers.csv` | per instance: best and mean over **60 runs of 2 hours** for each of the paper's 8 reference solvers |

**The CVRPLib archive ships the pre-challenge solutions.** All 100 `.sol` files
downloaded by `fetch_cvrplib.py` match the *initial* BKS column and none the
final one; 99 of the 100 were improved during the 30-day challenge, by 0.085 %
on average and up to 0.420 % (XL-n9571-k55). A gap measured against the archive
is therefore understated, which is why `gap_to_bks.py` prefers `final_bks` when
given `baseline/xl_bks.csv`, and prints which column it used. Run
`tools/parse_xl_bks.py --check` to see the comparison.

That table also puts the local runs in proportion. Gap to the final BKS, mean
over the 100 instances:

| method | budget | best | single run (mean of 60) |
|---|---|---:|---:|
| AILS-II | 2 h × 60 | +0.089 % | +0.159 % |
| FILO2 | 2 h × 60 | +0.203 % | +0.294 % |
| FILO | 2 h × 60 | +0.237 % | +0.334 % |
| SISRs | 2 h × 60 | +0.425 % | +0.583 % |
| KGLS^XXL | 2 h × 60 | +0.879 % | +1.090 % |
| HGS-CVRP | 2 h × 60 | +1.201 % | +1.448 % |
| **SAVANT** | **18 CPU-s, 1 run** | — | **+2.073 %** |
| HGS-CVRP, run here | 20 CPU-s, 1 run | — | +4.365 % *(97/100)* |
| LKH-3 | 2 h × 60 | +5.560 % | +6.746 % *(90/100)* |
| OR-Tools | 2 h × 60 | +5.628 % | +6.902 % |

Two readings, and the second is the honest one.

The flattering one: at **18 CPU-seconds** SAVANT lands within 0.6 points of
HGS-CVRP given **two hours** — a 400× budget — and beats LKH-3 and OR-Tools at
two hours by more than four points.

The sober one: **the field is far ahead of both.** AILS-II at +0.159 % is an
order of magnitude closer to the BKS than SAVANT is, and the specialised
large-scale methods (FILO, FILO2, SISRs) are all under +0.6 %. SAVANT beating
HGS on XL is a statement about HGS's anytime behaviour at short budgets, not
about SAVANT being competitive on large instances. Any XL claim needs the
budget attached and these rows next to it.

One corroboration worth noting: the paper's own footnote records that **LKH-3
found no feasible solution for 10 instances**, and fewer than 60 feasible runs
for 10 more. The feasibility failures seen here with HGS are the same
phenomenon on the same instance family, not an artefact of the local setup.

### Two more reference solvers: LKH-3 and AILS-II

Both are driven into the same run-directory layout as HGS, so `validate.py`
recomputes every cost they produce from the coordinates and `gap_to_bks.py`
scores them against the same references.

`sh tools/setup.sh solvers` builds both. By hand:

```bash
# LKH-3 (Helsgaun) — C, builds in one command
curl -O http://webhotel4.ruc.dk/~keld/research/LKH-3/LKH-3.0.14.tgz
tar xzf LKH-3.0.14.tgz -C external/ && make -C external/LKH-3.0.14
python3 tools/run_lkh.py --dir data/cvrplib/X --time 10 --jobs 12

# AILS-II (Máximo, Cordeau, Nascimento) — Java, source only, needs a jar.
# Any JDK 11+ will do; sh tools/setup.sh jdk fetches one if there is no root.
...
First 24 Set X instances, ~10 CPU-seconds each, every run validated with 0
errors and all 24 solved:

| solver | cpu s/inst. | gap to BKS |
|---|---:|---:|
| AILS-II | 9.63 | **+0.099 %** |
| HGS-CVRP | 10.00 | +0.244 % |
| SAVANT | 9.51 | +0.573 % |
| LKH-3 | 10.05 | +2.895 % |

The ordering matches the paper's, which is the point of running them: AILS-II
ahead of HGS, LKH-3 far behind. It also places SAVANT honestly — third of four
at this size, closer to HGS than to AILS-II.

#### Four traps, one per solver

**AILS-II: never pass `-best`.** It is a stopping criterion, not an
annotation — `bestF <= optimal` ends the search. Handing it a BKS makes it stop
the instant it matches, and every timing becomes meaningless. Its default of 0
is unreachable, so `run_ails.py` simply never passes it.

**AILS-II's `-limit` is wall clock**, not CPU (it polls
`System.currentTimeMillis`). Unlike HGS's `-t`, running 12 at once therefore
gives each less real work than a solo run. Both figures are recorded; `--jobs 1`
is the honest setting for a timing comparison.

**LKH-3: `MAX_TRIALS` binds before `TIME_LIMIT`.** Its default is `DIMENSION`,
so a nominal 10-second budget ended after 2.1 s on Set X, at +4.01 % instead of
+2.90 %. `run_lkh.py` therefore raises `MAX_TRIALS` to 10⁶ whenever `--time` is
given, so the time limit is what actually stops the run.

**LKH-3: `SPECIAL` is not optional for CVRP.** It selects LKH-3's purpose-built
moves. Without it, 68 s on X-n101-k25 never reached a feasible solution; with
it, feasible in 4 s. And `VEHICLES` is a fixed route count, not a bound — the
paper takes it from the `k` in the instance name, and so does this driver,
falling back to the bin-packing bound when the name carries none.

Two consequences worth stating plainly. LKH-3 reports `Cost = P_C` where a
non-zero `P` means **capacity is violated** — it still writes the file and exits
0, so the penalty is checked and a penalised answer is recorded as a failure
rather than averaged in. And LKH-3 computes EUC_2D by TSPLIB rules with no
float mode, so it cannot run the continuous NeuOpt instances at all;
`run_lkh.py` refuses them rather than silently answering a different problem.
AILS-II has `-rounded`, so it runs both families natively.

#### The AILS-II patch

AILS-II prints only a cost line. Left alone, its numbers would be the only ones
in this repository taken on trust — no coverage check, no capacity check, no
recomputed cost. `tools/ails_solution_output.patch` adds a `-solution <file>`
option, and nothing else: it reads one extra argument from `argv` inside
`main()` and writes through the `Solution.toString()` that already exists,
which happens to emit exactly the CVRPLib `.sol` format with the depot as node
0 and customers 1..n. No search parameter, acceptance rule or operator is
touched, so the trajectory is identical to the unpatched code. The run's
`config.json` records that the patch was applied.

It is a real unified diff, with line numbers and space-prefixed context, so GNU
`patch`, BSD `patch` and `git apply -p0` all accept it. (An earlier version used
a bare `@@` with no ranges — which BSD `patch` tolerates and GNU `patch` rejects
outright as "only garbage was found in the patch input".)
...
with. It prints the difference twice: as a ratio of the two means, and as the mean
of the per-instance relative gaps. On a uniform set the two agree; on Set X, whose
costs span 11 000 to 337 000, the first is dominated by the largest instances and
only the second answers "which solver is better here".
...
One of its cases drives `cw --dir data/cvrp_200`, the TSPLIB-directory input
path, which only `fetch_neuopt.py --tsplib` writes. `setup.sh` therefore
generates that directory too (12 MB); without it 10 of the 200 trials fail on a
missing directory rather than on anything about the solver.
...
| `Makefile`         | `make auto` (probes for OpenMP, picks a target), `make`, `make macos` (Apple clang + libomp), `make serial` (no OpenMP), `make repro` (bit-reproducible), `make debug` (ASan/UBSan) |
...
| `tools/fetch_cvrplib.py` | download CVRPLib X / XL / XML100, their references, and verify the rounding convention |
| `tools/run_hgs.py` | runs HGS-CVRP into the same run-directory layout, so `validate.py` checks it too |
| `tools/run_lkh.py` | the same for LKH-3 (`SPECIAL`, fixed `VEHICLES`, penalty checked) |
| `tools/run_ails.py` | the same for AILS-II (Java; never passes `-best`) |
| `tools/ails_solution_output.patch` | adds `-solution <file>` to AILS-II so its routes can be revalidated |
...
| `tools/gap_to_bks.py` | gap of a run to the CVRPLib optima / best known solutions      |
| `tools/parse_xl_bks.py` | extracts the XL paper's BKS and reference-solver tables into `baseline/` |
| `tools/setup.sh`   | checks the toolchain, fetches every dataset, builds every solver (idempotent) |
| `tools/compare_all.sh` | runs all four solvers on one CVRPLib set, validates them and scores them against its BKS |
| `tools/compare_hgs.sh` | the SAVANT-vs-HGS budget sweep on the NeuOpt sets             |
| `tools/compare_cvrplib.sh` | the same on a CVRPLib set, scored against its reference   |
...
| `baseline/`        | published baselines (`baseline.csv`, `xl_bks.csv`, `xl_solvers.csv`) and their LaTeX rendering |
# SAVANT

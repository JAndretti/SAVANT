...
`setup.sh` is idempotent — anything already present is left alone — and it
degrades rather than failing: without `libomp` it builds SAVANT single-threaded,
without a JDK it skips AILS-II and says so.

#### Requirements

| tool | needed for | if missing |
|---|---|---|
| C compiler, `make` | SAVANT, LKH-3 | Xcode command line tools / `build-essential` |
| `cmake` | HGS-CVRP | `brew install cmake` |
| `curl`, `git` | fetching everything | — |
| `bsdtar` | reads the `.7z` archives natively | ships with macOS; Debian: `apt install libarchive-tools` |
| `python3` + `uv` | the `tools/` scripts | plain `python3` works too; no third-party packages are needed except `pypdf`, and only for `parse_xl_bks.py` |
| `libomp` | SAVANT's OpenMP loop **(optional)** | `brew install libomp`; otherwise `make serial` |
| a JDK | AILS-II **(optional)** | `brew install openjdk` / `apt install default-jdk` |

Disk: ~300 MB of instances (10 000 NeuOpt × 3 sizes, 10 000 XML100, X, XL) and
~100 MB of solver sources. Nothing under `data/`, `external/` or `results/` is
tracked by git; all of it is regenerated by `setup.sh`.

...
def split_naive(tour, dem, cap, d):
    ...
    n = len(tour)
    P = [float('inf')] * (n + 1); P[0] = 0.0
    pred = [0] * (n + 1)
    for i in range(n):
        load = 0.0; cost = 0.0
        for j in range(i + 1, n + 1):
            load += dem[tour[j - 1]]
            if load > cap + 1e-9: break
            if j == i + 1:
                cost = 2 * d(0, tour[i])
            else:
                cost = (cost - d(tour[j - 2], 0) + d(tour[j - 2], tour[j - 1])
                        + d(tour[j - 1], 0))
            if P[i] + cost < P[j] - 1e-12:
                P[j] = P[i] + cost; pred[j] = i
    return P[n]
...
if __name__ == "__main__":
    ...
    subprocess.run(["./cw", "--bundle", bundle, "--limit", str(lim), "-q",
                    "--split", "off", "--sol", "/tmp/s0.txt", "--csv", "/tmp/c0.csv"]
                   + extra, capture_output=True, text=True)
    # same solution WITH a final split
    subprocess.run(["./cw", "--bundle", bundle, "--limit", str(lim), "-q",
                    "--split", "end", "--split-tour", "routes", "--csv", "/tmp/c1.csv"] + extra,
    ...
    for k, (n, cap, xs, ys, ds) in enumerate(insts):
        ...
        worst = max(worst, rel)
        print(f"inst {k:2d}: split C={c1[k]:.6f}  naive DP={ref:.6f}  "

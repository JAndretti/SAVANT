...
def cw_naive(n, cap, xs, ys, ds):
    ...
    for r in route.values():
        cost += d(0, r[0]) + sum(d(r[t], r[t+1]) for t in range(len(r)-1)) + d(r[-1], 0)
...
def check_sol(solfile, insts):
    ...
    for k, (n, cap, xs, ys, ds) in enumerate(insts):
        ...
        assert seen == list(range(1, n+1)), f"instance {k}: missing/duplicated customers"
        for r in rs:
            assert sum(ds[c] for c in r) <= cap + 1e-9, f"instance {k}: capacity exceeded"
    return len(insts)
...
if __name__ == "__main__":
    ...
    out = subprocess.run(["./cw", "--bundle", bundle, "--limit", str(limit),
                          "--exact", "--per-instance", "-q", "--sol", "/tmp/sol.txt"],
                         capture_output=True, text=True)
    print(out.stdout.strip().splitlines()[-1] if out.stdout else out.stderr)
    csv = subprocess.run(["./cw", "--bundle", bundle, "--limit", str(limit),
                          "--exact", "-q", "--csv", "/tmp/r.csv"], capture_output=True, text=True)
